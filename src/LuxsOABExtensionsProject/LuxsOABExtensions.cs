...
public class LuxsOABExtensions : BaseSpaceWarpPlugin
{
    ...
    void LoadConfigs()
    {
        ...
        foreach (string path in sizesPaths)
        {
            ...
            using (StreamReader sr = new(path))
            {
                ...
                {
                    ModLogger.LogWarning($"{size.FullName} ({size.AbbreviatedName}) already exists! ({path})");
                }
            }
        }
        ...
        foreach (var path in partsPaths)
        {
            ...
            using (StreamReader sr = new(path.FullName))
            {
                ...
                foreach (LOABEPartData loabePartData in loabePartDataCollection.LOABEPartData)
                {
                    ...
                    {
                        ModLogger.LogWarning($"{path.FullName} is searching for a size that doesnt exist ({loabePartData.LOABESizeID})");
                        errorCount++;
                    }
                }
                if (errorCount > 0)
                    ModLogger.LogWarning($"{loabePartDataCollection.mod}'s loading finished with {errorCount} errors");
                ModLogger.LogInfo($"{loabePartDataCollection.mod} applied LOABEConfigs to {partsApplied} parts");
            }
        }
    }
    ...
    private List<(string name, UnityObject asset)> ImportConfig(string internalName, string filename)
    {
        ...
        object LOABEConfig = JsonConvert.DeserializeObject(rawData);

        switch (LOABEConfig)
        {
            case LOABESizeData loabeSizeData:
                CustomSizes.Add(loabeSizeData.ToLOABESize());
                break;
            case LOABEPartData loabePartData:
                if (loabePartData.TryGetSize(out var size))
                    loabePartData.partNames.ForEach(a => overrides.Add(a, size));
                break;
        }
    ...
    public static int SymmetryValue = 1;
    public static bool overrideSymmetry = false;
    private static int lastSymmetry = 1;
    private static bool lastOverrideSymmetry = false;
    ...
    private static void FillWindow(int windowID)
    {
        ...
        overrideSymmetry = GUILayout.Toggle(overrideSymmetry, "Override default symmetry?");

        if (overrideSymmetry)
        {
            int SymmetryValue = LuxsOABExtensions.SymmetryValue;
            ...
            if (lastSymmetry != SymmetryValue)
            {
                lastSymmetry = SymmetryValue;
                LuxsOABExtensions.SymmetryValue = SymmetryValue;
                GameObject.FindObjectOfType<ObjectAssemblyToolbar>().SetSymmetryToolByEnum((BuilderSymmetryMode)SymmetryValue);
                int i = Array.Find(RegularSymmetryModes, a => a == SymmetryValue);
                if (i != -1)
                    UpdateSprite();
            }
            GUILayout.Space(10);
        ...
        if (lastOverrideSymmetry != overrideSymmetry)
        {
            lastOverrideSymmetry = overrideSymmetry;
            if (!overrideSymmetry)
            {
                int i = Array.Find(RegularSymmetryModes, a => a == SymmetryValue);
                if (i == -1)
